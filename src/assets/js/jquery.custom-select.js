...
/* eslint-disable no-undef */ $(function () {
    $.fn.CustomSelect = function (c) {
        var h = {
            ...
            b = { UP: 38, DOWN: 40, LEFT: 37, RIGHT: 39, ENTER: 13, SPACE: 32, ESC: 27 },
            d = $.extend({}, h, c || {});
        d.stateClasses = {
        ...
        this.each(function () {
            ...
            var H = $('<div>', {
                    ...
                    html: '<i class="' + d.prefix + '__title__icon fa fa-angle-down"></i>',
                }),
                ...
                q = $('<div>', {
                    class:
                        d.prefix +
                        (typeof d.modifier == 'string' && d.modifier != '' ? ' ' + d.prefix + '_' + d.modifier : ''),
                    html: H,
                }),
                ...
                u = $('<div>', { class: d.prefix + '__dropdown__inner' }).css({ height: '100%', 'min-width': '100px' }),
                k = $('<div>', { class: d.prefix + '__scrollbar' }),
                m = $('<div>', { class: d.prefix + '__wrap-scrollbar', html: k });
            s.css({ display: 'none', position: 'absolute', width: z + m.width(), zIndex: 999 });
            l.css({ width: z + m.width() - $('.' + d.prefix + '__title__icon fa fa-angle-down').outerWidth() });
            q.css({ width: z + m.width() });
            B.usekey = false;
            B.wrap(u);
            u = $('.' + d.prefix + '__dropdown__inner', s);
            n();
            E.css({ position: 'absolute', opacity: 0.1, width: 0, height: 0 }).after(q);
            ...
            function n() {
                ...
                (p = E.attr('hidden') == 'hidden'),
                    $('option', E).each(function (K) {
                        var M = $(this),
                            I = M.data('modifier'),
                            J = $('<div>', { class: d.prefix + '__item', title: M.text(), text: M.text() });
                        if (I) {
                            J.addClass(d.prefix + '__item_' + I);
                        }
                        if (M.is(':selected') && M.val() != i && i != 'undefined') {
                            v(M.text());
                            J.addClass(d.stateClasses.itemSelected).siblings().removeClass(d.stateClasses.itemSelected);
                        }
                        if (M.is(':disabled')) {
                            ...
                            J.on('click', function (N) {
                                ...
                                if (o) {
                                    ...
                                    v('Выбрано [' + $('.' + d.stateClasses.itemSelected, B).length + ']');
                                    E.change();
                                } else {
                                    if (!$(this).hasClass(d.stateClasses.itemSelected)) {
                                        $(this)
                ...
                if (o) {
                    ...
                    if (typeof i == 'undefined' || $('.' + d.stateClasses.itemSelected, B).length > 0) {
                        i = 'Выбрано [' + $('.' + d.stateClasses.itemSelected, B).length + ']';
                    }
                }
                v(i);
            }
            function r(Q) {
                ...
                if (J < M && L <= K) {
                    ...
                    if (J > M && K >= P - L) {
                        I = u.scrollTop() + L + K - P + parseInt(B.css('padding-bottom'), 10);
                    }
                }
                O.click();
                if (I >= 0) {
            ...
            function y() {
                return (u.scrollTop() / (B.outerHeight(true) - u.height())) * (u.height() - k.outerHeight(true));
            }
            function C(J) {
                var I,
                    L = 0,
