...
function custom_select() {
    if ($('.htlfndr-select-custom').length > 0) {
        $('.htlfndr-select-custom').CustomSelect({ visRows: 4, search: true, modifier: 'mod' });
    }
        ...
        function (d) {
            ...
            switch (e.id) {
                case 'htlfndr-sort-by-price':
                    tinysort('.htlfndr-hotel-post-wrapper', { selector: '.htlfndr-hotel-price>.cost' });
                    break;
                ...
                case 'htlfndr-sort-by-popular':
                    tinysort('.htlfndr-hotel-post-wrapper', { selector: '.htlfndr-hotel-reviews>span', order: 'desc' });
                    break;
                default:
            }
        },
...
function browser_width() {
    ...
    if (a < 1280) {
        ...
        $('.pagination').on('click', '.htlfndr-right,.htlfndr-left', function () {
            $(this).find('a').css({ 'background-color': '#08c1da', color: '#fff' }).delay(400);
            $(this).find('a').animate({ 'background-color': 'transparent', color: '#000000' }, 200);
        });
    }
    if (a < 1280 && a > 767) {
        $('.dropdown-submenu').on('click', '>a', function () {
...
function slider() {
    if ($('.htlfndr-slider-section').length) {
        $('.owl-carousel').owlCarousel({ singleItem: true, autoPlay: 3000, pagination: false, paginationSpeed: 1000 });
    }
    if ($('#htlfndr-room-slider').length) {
        $('.owl-carousel').owlCarousel({ singleItem: true, autoPlay: 60000, pagination: true, paginationSpeed: 1000 });
    }
...
(function (b) {
    ...
    if (a < 1199) {
        b('.htlfndr-user-page .htlfndr-hotel-post-wrapper').css('margin-top', '-20px');
    }
})(jQuery);
